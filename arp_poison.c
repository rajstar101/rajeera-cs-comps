...
// this function converts the MAC address from string to uint8_t
void convert_mac(const char *mac_str, uint8_t *mac) {
    ...
    // convert the MAC address from string to uint8_t
    int values[6];
    ...
    // store the values in the uint8_t array
...
int main(int argc, char* argv[]) {
    ...
    if (target_mac == NULL){ // check if the memory allocation was successful
        perror("malloc");
        return 1;
    ...
    if (ethernet == NULL) { // check if the memory allocation was successful
        perror("malloc");
        free(target_mac);
        return 1;
    }
